...
function showNotes() {
  ...
  notes.forEach((note, idx) => {
    let card = `
        ...
        <div class="menu-app">
            <i class="fas close fa-bars"></i>
            <ul class="menu">
              <li onclick="editNote(${idx}, '${note.title}', '${note.description}')"><i class="fas close fa-gear"></i> تعديل</li>
              <li onclick="removeNote(${idx})"><i class="fas close fa-trash"></i> حذف</li>
            </ul>
          </div>
        </div>
      </div>
...
function removeNote(idNote) {
  Swal.fire({
    title: 'هل انت متأكد من حذف الملحوظة؟',
    ...
    if (result.isConfirmed) {
      ...
      localStorage.setItem("notes", JSON.stringify(notes));
      showNotes();
      Swal.fire('تم الحذف!', 'تم حذف الملحوظة بنجاح.', 'success');
      audio.play();
    }
  });
}
